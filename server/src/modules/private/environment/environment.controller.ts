...
class EnvironmentController {
    ...
    getEnvironmentStatus = async (req: Request, res: Response) => {
        ...
        const status = {
            ...
            hasFfmpeg,
            hasFfprobe,
            hasDeepgramKey: !!(uDoc?.deepgramKey || process.env.DEEPGRAM_API_KEY),
            hasAnthropicKey: !!(uDoc?.anthropicKey || process.env.ANTHROPIC_API_KEY),
            hasDeepseekKey: !!(uDoc?.deepseekKey || process.env.DEEPSEEK_API_KEY),
            hasGeminiKey: !!(uDoc?.geminiKey || process.env.GEMINI_API_KEY),
            hasOpenaiKey: !!(uDoc?.openaiKey || process.env.OPENAI_API_KEY),
            hasOpenrouterKey: !!(uDoc?.openrouterKey || process.env.OPENROUTER_API_KEY),
            hasGroqKey: !!(uDoc?.groqKey || process.env.GROQ_API_KEY),
            hasMistralKey: !!(uDoc?.mistralKey || process.env.MISTRAL_API_KEY || process.env.MIXTRAL_API_KEY),
