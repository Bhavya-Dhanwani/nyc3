...
export function App() {
    ...
    async function loadProjectDetails() {
      try {
        ...
        if (isMounted && timelineRes.data?.data) {
          const { project } = timelineRes.data.data;
        ...
        void api.get(`/api/projects/${pId}/drive-files`)
          .then((driveRes) => {
            if (isMounted && driveRes.data?.data?.files) setDriveFiles(driveRes.data.data.files);
          })
          .catch(() => {});
  ...
  const handleSaveProjectToBackend = async () => {
    ...
    try {
      ...
      notify("Project timeline saved to AutoShorts cloud!");
    } catch (err) {
      console.error("Save timeline error:", err);
      notify("Failed to save project timeline to cloud");
    } finally {
      setIsSavingToBackend(false);
    }
  };

  const handleLoadMomentIntoTimeline = async ({ start, end, title, aspectRatio }) => {
    ...
    // 4. Load captions for this clip from the project transcript
    try {
      const pId = currentProject?._id || currentProject?.id;
      if (pId) {
        ...
        if (transcript) {
          ...
          if (clipWords.length > 0) {
            ...
            if (formatted.length > 0) {
              ...
              notify(`Loaded ${formatted.length} Hinglish captions for "${title || 'Clip'}" (${clipDuration.toFixed(1)}s)`);
              return;
            }
          }
        }
