...
export function PreviewStage({
  ...
  const [inlineFrameSize, setInlineFrameSize] = useState({ width: 0, height: 0 });
  ...
  const activePreviewFrameSize = isFocusPreviewOpen && focusPreviewFrameSize.width > 0
    ? focusPreviewFrameSize
    : previewFrameSize?.width > 0 && previewFrameSize?.height > 0
      ? previewFrameSize
      : inlineFrameSize.width > 0 && inlineFrameSize.height > 0
        ? inlineFrameSize
        : { width: 0, height: 0 };
  const activePreviewFrameStyle = activePreviewFrameSize.width > 0
    ? { ...previewFrameStyle, width: `${activePreviewFrameSize.width}px`, height: `${activePreviewFrameSize.height}px` }
    : previewFrameStyle;
  ...
  useEffect(() => {
    ...
    const updateFrameSize = () => {
      ...
      if (isFocusPreviewOpen) {
        setFocusPreviewFrameSize((current) => current.width === width && current.height === height ? current : { width, height });
      } else {
        setInlineFrameSize((current) => current.width === width && current.height === height ? current : { width, height });
      }
    };
    updateFrameSize();
    const observer = window.ResizeObserver ? new ResizeObserver(updateFrameSize) : null;
