...
export function GlobalFileDropOverlay({ isDragging, t }) {
  if (!isDragging) return null;
  return <div className="global-file-drop-overlay">
    <div className="global-file-drop-content">
      <svg width="48" height="48" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
      <h2>{t?.("dropToImport") || "Drop to Import Media"}</h2>
      <p>{t?.("dropToImportSub") || "Release to add files to your library"}</p>
    </div>
  </div>;
}

export function KeyboardShortcutsModal({ open, onClose, t }) {
  if (!open) return null;
  const shortcuts = [
    { key: "Space", desc: t?.("shortcutPlay") || "Play / Pause" },
    { key: "Delete / Backspace", desc: t?.("shortcutDelete") || "Delete Selected Clip" },
    { key: "Cmd/Ctrl + Z", desc: t?.("shortcutUndo") || "Undo" },
    { key: "Cmd/Ctrl + Shift + Z", desc: t?.("shortcutRedo") || "Redo" },
    { key: "Shift + ?", desc: t?.("shortcutHelp") || "Show Shortcuts" },
  ];
  return <div className="shortcut-modal-overlay" onClick={onClose}>
    <div className="shortcut-modal-card" onClick={e => e.stopPropagation()}>
      <header>
        <h3>{t?.("keyboardShortcuts") || "Keyboard Shortcuts"}</h3>
