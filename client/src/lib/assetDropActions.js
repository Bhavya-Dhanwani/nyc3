import { decodeWaveform, extractVideoThumbnail, extractVideoTrackFrames } from "./media.js";
...
export function createAssetDropActions(d) {
  ...
  async function applyAssetToTrack(rawAsset, track, options = {}) {
    if (!d.canDropAssetOnTrack(rawAsset, track)) {
      d.notify(tr("assetTrackMismatch", "请把素材拖到匹配的轨道"));
      return;
    }
    let pendingSegment = null;
    const hadVisualBefore = Boolean(d.visualSegments.length || d.imageSrc);
    let progressBucket = -1;
    const asset = await resolveRemoteAsset(rawAsset, rawAsset?.blob ? undefined : (progress) => {
      const bucket = Math.min(0.95, Math.floor(progress * 20) / 20);
      if (bucket <= progressBucket) return;
      if (!pendingSegment && (track === "image" || !hadVisualBefore)) {
    ...
    if (track === "image") {
      if (pendingSegment) {
        const initialThumb = asset.type === "video" ? await extractVideoThumbnail(asset.src, { width: asset.width, height: asset.height }) : "";
        d.updateVisualAssetInTimeline(asset.id, {
          ...asset,
          thumbnail: initialThumb || asset.thumbnail || "",
          trackFrames: initialThumb ? [initialThumb] : (asset.trackFrames || []),
          preparing: false,
          prepareProgress: 1,
        });
        if (asset.type === "video") {
          extractVideoTrackFrames(asset.src, {
            duration: asset.duration,
            width: asset.width,
            height: asset.height,
            onBatch: (trackFrames) => {
              if (trackFrames.length) d.updateVisualAssetInTimeline(asset.id, { trackFrames });
            },
          })
