...
export function seekVideoFrame(video, time, timeoutMs = 1500) {
...
export async function extractVideoThumbnail(src, options = {}) {
  try {
    const video = await loadVideo(src);
    const canvas = document.createElement("canvas");
    canvas.height = VIDEO_TRACK_FRAME_HEIGHT;
    const naturalWidth = Math.max(1, options.width || video.videoWidth || 16);
    const naturalHeight = Math.max(1, options.height || video.videoHeight || 9);
    ...
    await seekVideoFrame(video, 0.001);
    context.drawImage(video, 0, 0, canvas.width, canvas.height);
    const frame = canvas.toDataURL("image/jpeg", options.quality || 0.72);
    video.removeAttribute("src");
    video.load();
    return frame;
  } catch (error) {
    console.warn("Failed to extract quick video thumbnail", error);
    return "";
  }
}
...
export async function extractVideoTrackFrames(src, options = {}) {
  ...
  try {
    ...
    for (let index = 0; index < frameCount; index += 1) {
      ...
      if (onBatch && (index === 0 || index === 2 || (index + 1) % 5 === 0 || index === frameCount - 1)) {
        try {
          onBatch([...frames]);
        } catch {
          // ignore batch notification errors
        }
      }
    }
    return frames;
