...
export function useTimelineModel(d) {
  ...
  // Captions without explicit SRT timestamps used to be sized only against
  // generated voice-over. When editing a screen recording (or a silent visual)
  // that value is zero, so captions end long before the visual and appear to
  // vanish in both the preview and an export. Use the complete active timeline
  // as the shared fallback instead.
  const captionTargetDuration = useMemo(() => Math.max(
    voiceTrackDuration,
    d.sourceAudioBlob ? finiteDuration(d.sourceAudioTimelineEnd) : 0,
    d.imageSrc ? finiteDuration(d.imageDuration) : 0,
  ), [
    d.imageDuration, d.imageSrc, d.sourceAudioBlob, d.sourceAudioTimelineEnd,
    voiceTrackDuration,
  ]);
